...
from eventyay.common.utils import daterange, path_with_hash
...
class Event(PretalxModel):
    ...
    def build_initial_data(self):
        from eventyay.mail.default_templates import (
            ACCEPT_TEXT,
            ACK_TEXT,
            GENERIC_SUBJECT,
            QUESTION_SUBJECT,
        ...
        from eventyay.submission.models import CfP
        ...
        if not self.schedules.filter(version__isnull=True).exists():
            from eventyay.schedule.models import Schedule
        ...
        if not self.review_phases.all().exists():
            from eventyay.submission.models import ReviewPhase
        ...
        if not self.score_categories.all().exists():
            from eventyay.submission.models import ReviewScore, ReviewScoreCategory
    ...
    def copy_data_from(self, other_event, skip_attributes=None):
        from eventyay.orga.signals import event_copy_data
    ...
    def wip_schedule(self):
        ...
        try:
            ...
            from eventyay.schedule.models import TalkSlot
    ...
    def get_mail_backend(self, force_custom: bool = False) -> BaseEmailBackend:
        from eventyay.common.mail import CustomSMTPBackend
