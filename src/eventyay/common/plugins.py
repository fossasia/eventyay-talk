...
def get_all_plugins(event=None):
    """Return the EventyayPluginMeta classes of all plugins found in the
    ...
    return sorted(
        plugins,
        key=lambda m: (
            0 if m.module.startswith("eventyay.") else 1,
            str(m.name).lower().replace("eventyay ", ""),
        ),
    )
...
def get_all_plugins_grouped(event=None, filter_visible=True):
    ...
    plugins_grouped = {
        category: sorted(
            plugins, key=lambda p: str(p.name).lower().replace("eventyay ", "")
        )
