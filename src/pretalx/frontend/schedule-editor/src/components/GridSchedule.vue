...
const emit = defineEmits(['startDragging', 'rescheduleSession', 'createSession', 'editSession', 'changeDay'])
...
const visibleTimeslices = computed(() => {
	return timeslices.value.filter(slice => {
		return slice.date.minute() % 30 === 0 || expandedTimes.value.includes(slice.date) || oddTimeslices.value.includes(slice.date)
	})
})
...
const availabilitySlices = computed(() => {
	...
	for (const room of visibleRooms.value) {
		if (!props.availabilities.rooms[room.id] || !props.availabilities.rooms[room.id].length) avails.push({room: room, start: earliestStart, end: latestEnd})
		else {
...
const staticOffsetTop = computed(() => {
	const rect = grid.value?.parentElement.getBoundingClientRect()
	return rect?.top || 0
})

const scrollParent = computed(() => grid.value?.parentElement)

const visibleRooms = computed(() => props.rooms.filter(room => !hiddenRooms.value.includes(room)))

const visibleSessions = computed(() => props.sessions.filter(session => !hiddenRooms.value.includes(session.room)))
...
onMounted(async () => {
  ...
  // Observe all datebreak slices using class selector
  const datebreakSlices = grid.value?.querySelectorAll('.timeslice.datebreak')
  if (datebreakSlices) {
    datebreakSlices.forEach(el => {
...
function expandTimeslice(slice) {
...
function updateHoverSlice(e) {
	...
	const roomWidth = document.querySelectorAll('.grid .room')[1]?.getBoundingClientRect().width || 0
...
function getSliceClasses(slice) {
	return {
...
function isSliceExpandable(slice) {
	const index = visibleTimeslices.value.indexOf(slice)
...
function changeDay(day) {
	...
	const el = document.querySelector(`[data-slice="${day.format()}"]`)
	if (!el) return
...
function dragOnScroll() {
	...
	if (!event || !scrollParent.value) return
	if (event.clientY - staticOffsetTop.value < 160) {
		if (event.clientY - staticOffsetTop.value < 90) {
			scrollBy(-200)
		} else {
			scrollBy(-75)
		}
	} else if (event.clientY > scrollParent.value.clientHeight + staticOffsetTop.value - 100) {
		if (event.clientY > scrollParent.value.clientHeight + staticOffsetTop.value - 40) {
			scrollBy(200)
		} else {
			scrollBy(75)
		}
